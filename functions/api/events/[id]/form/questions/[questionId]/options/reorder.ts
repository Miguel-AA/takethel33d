// POST /api/events/:id/form/questions/:questionId/options/reorder
//
// A static segment, so it takes precedence over `[optionId]`.

import { error } from '../../../../../../../_shared/responses';
import { reorderFormOptionsSchema } from '../../../../../../../../shared/schemas';
...
export const onRequestPost: PagesFunction<Env, 'id' | 'questionId', AdminRequestData> = async (
  ...
  const parsed = reorderFormOptionsSchema.safeParse(body.value);
  if (!parsed.success) {
    return error(
      400,
      'VALIDATION_ERROR',
