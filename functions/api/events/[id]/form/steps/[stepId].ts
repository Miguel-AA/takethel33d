// PATCH  /api/events/:id/form/steps/:stepId — rename or re-describe a page
// DELETE /api/events/:id/form/steps/:stepId — remove an EMPTY page
...
export const onRequestPatch: PagesFunction<Env, 'id' | 'stepId', AdminRequestData> = async (
  ctx,
) => {
  const { admin, requestContext, requestId } = adminContext(ctx.data, ctx.request);

  const path = parseFormPath(ctx.params as Record<string, string>, 'id', 'stepId');
  if (!path) {
  ...
  const parsed = updateFormStepSchema.safeParse(body.value);
  if (!parsed.success) {
  ...
  const result = await service.updateStep(path.id, path.stepId, parsed.data, {
    admin,
    requestContext,
  });
...
export const onRequestDelete: PagesFunction<Env, 'id' | 'stepId', AdminRequestData> = async (
  ctx,
) => {
  const { admin, requestContext, requestId } = adminContext(ctx.data, ctx.request);
  ...
  if (!parsed.success) {
    return error(400, 'VALIDATION_ERROR', 'Invalid payload', validationFields(parsed.error.issues), {
      requestId,
    });
  }
