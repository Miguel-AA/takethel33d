// POST /api/events/:id/form/steps/reorder — a complete new page order
//
// A static segment, so it takes precedence over `[stepId]` and a step can never
// be named "reorder" in a way that shadows it.

import { error } from '../../../../../_shared/responses';
import { reorderFormStepsSchema } from '../../../../../../shared/schemas';
...
export const onRequestPost: PagesFunction<Env, 'id', AdminRequestData> = async (ctx) => {
  ...
  const parsed = reorderFormStepsSchema.safeParse(body.value);
  if (!parsed.success) {
    return error(400, 'VALIDATION_ERROR', 'Invalid reorder payload', validationFields(parsed.error.issues), {
      requestId,
    });
  }
