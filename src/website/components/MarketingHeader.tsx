...
export function MarketingHeader({ nav }: { nav: LandingCopy['nav'] }) {
  ...
  const links = [
    ...
    { to: '/benefits', label: nav.benefits },
    { to: '/how-it-works', label: nav.how },
    { to: '/industries', label: nav.industries },
    ...
    <header className="fixed inset-x-0 top-0 z-40 border-b border-white/40 bg-white/55 backdrop-blur-2xl supports-[backdrop-filter]:bg-white/40">
      ...
      {open && (
        <nav
          ...
          className="border-t border-white/40 bg-white/75 backdrop-blur-2xl lg:hidden"
          aria-label="Páginas del sitio"
        >
          <ul className="section-x flex flex-col gap-1 py-3">
            {links.map((l) => (
              <li key={l.to}>
            ...
            <li className="mt-2 flex flex-col gap-2 border-t border-white/40 pt-3">
              <Link to="/contact" onClick={() => setOpen(false)} className="btn-primary px-4 py-2.5 text-sm">
                {nav.getLeads}
              </Link>
              <Link to="/events" onClick={() => setOpen(false)} className="btn-secondary px-4 py-2.5 text-sm">
                {nav.events}
              </Link>
            </li>
          </ul>
